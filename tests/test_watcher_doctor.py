...
def test_lmstudio_embedding_model_aliases():
    available_models = ["text-embedding-monas-embeddings-code"]

    assert is_lmstudio_embedding_model_available(
        "monas-embeddings-code", available_models
    )
    assert is_lmstudio_embedding_model_available(
        "text-embedding-monas-embeddings-code", available_models
    )
...
def test_watcher_filtering():
    ...
    handler.on_any_event(
        DummyEvent("D:/project/FourTIndex/node_modules/package/index.js")
    )
    ...
    time.sleep(0.2)
    assert (
        len(callback_called) == 1
    ), "Callback should be triggered for valid file modifications"
...
def test_doctor_diagnostics(mock_check):
    ...
    mock_check.side_effect = lambda url, *args, **kwargs: (
        True,
        "Online",
        ["monas", "text-embedding-monas-embeddings-code"],
    )
    ...
    with patch.object(Config, "llm_provider", "lmstudio"):
        with patch("sqlite3.connect") as mock_sql:
            ...
            with patch("src.doctor.create_provider") as mock_create:
                ...
                assert (
                    status is True
                ), "Diagnostics should succeed when dependencies are online"
...
def test_smart_context_guard(mock_count_tokens):
    ...
    mock_count_tokens.side_effect = lambda text, model: (
        20 if "--- USER QUERY ---" in text else 30
    )
    ...
    if llm.provider == "lmstudio":
        ...
        with patch.object(
            llm.client, "chat", return_value={"message": {"content": "ok"}}
        ) as mock_chat:
            llm.generate_answer("query", context)
    ...
    assert (
        "file2.py" not in prompt_sent
    ), "Second chunk should be pruned due to budget limit"
